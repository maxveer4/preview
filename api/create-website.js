...
module.exports = async function handler(req, res) {
  ...
  const logoHtml = foto_logo
    ? `<img src="${foto_logo}" alt="${bedrijfsnaam} logo" style="height:40px;width:auto;max-width:160px;object-fit:contain">`
  ...
  for (const [key, html] of Object.entries(templates)) {
    generated[PAGE_SLUG[key](slug)] = applyMap(html, map);
  }
  ...
  const klantRecord = {
    ...
    maps_url,
    template_keuze,
    template_homepage:  tplConfig.homepage  || '',
    template_diensten:  tplConfig.diensten  || '',
    template_contact:   tplConfig.contact   || '',
    template_over_ons:  tplConfig.over_ons  || '',
    template_projecten: tplConfig.projecten || '',
    sector:    beroep,
    kleur_thema,
    logo_url:       foto_logo,
    foto_hero:      fotoHero,
